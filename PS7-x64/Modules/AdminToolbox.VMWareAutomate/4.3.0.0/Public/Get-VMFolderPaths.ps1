Function Get-VMFolderPaths {
    ...
    ForEach ($folder in $folders) {
        Get-Folder -Name $folder | Get-VM | Select-Object Name | Export-Excel -WorksheetName "$Folder" -Path "$env:USERPROFILE\Downloads\VMFolderPaths.xlsx" -FreezeTopRow -TableName "$Folder" -AutoSize
    }
}